...
func (sc *LocalCache) Get(ctx context.Context, path, token string) (io.ReadCloser, error) {
	if !(sc.ac.authorize(token_scopes.Wlcg_Storage_Read, path, token) || sc.ac.authorize(token_scopes.Scitokens_Read, path, token)) {
...
func (lc *LocalCache) Stat(path, token string) (uint64, error) {
	if !(lc.ac.authorize(token_scopes.Wlcg_Storage_Read, path, token) || lc.ac.authorize(token_scopes.Scitokens_Read, path, token)) {
